...
> A full-stack system for hospitals and organisations to manage patient queues virtually. Patients reserve a slot online and **must physically check in** before entering the active queue — eliminating both no-shows and overcrowded waiting rooms.
...
| QR Check-In | Staff scan the patient's unique token QR at arrival |
| Manual Check-In | Receptionist types the token number as a fallback |
...
# Option B: open files directly in the browser
...
> 💡 In development, if SMTP is not configured, OTP codes are printed directly to the backend terminal — useful for testing.
...
| Method | Endpoint | Auth | Description |
|---|---|---|---|
| POST | `/checkin/qr` | ✅ Staff | Check in by scanning the patient's QR |
...
| Method | Endpoint | Auth | Description |
|---|---|---|---|
| POST | `/queue/call-next` | ✅ Staff | Call the next waiting patient |
...
| POST | `/queue/rejoin` | ✅ Receptionist | Create a new token after a no-show |
...
1. **Walk-in for returning patients** — if a patient was previously registered, the walk-in form fails on duplicate email. A patient lookup by phone/email before registration is not yet implemented in the UI.
2. **`orgSlug` hardcoded** — the patient portal currently uses `'city-hospital'` as the org slug. Update this if using a different organisation.
3. **Booking window is UTC** — the booking window enforcement runs in UTC. Enter your org config times in UTC.
4. **Staff management UI** — staff accounts can be created via the API (or MongoDB directly); an in-app admin form is in progress.
5. **Counter management UI** — counters can currently be created via the API; an in-app admin form is in progress.
